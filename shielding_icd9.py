...
def main(shielding_csv, mapping_csv, d_icd_diagnoses_csv, diagnosis_icd_csv):
    ...
    matched_shielding_diagnoses_df = shielding_diagnoses_df[~shielding_diagnoses_df.ROW_ID.isnull()]
    print()
    ...
    shielding_diagnoses_prevalence_df = matched_shielding_diagnoses_df[["ICD9"]].join(diagnosis_icd_df[["HADM_ID"]]).groupby(["ICD9"]).nunique()
    frequent_shielding_diagnoses_df = shielding_diagnoses_prevalence_df[shielding_diagnoses_prevalence_df.HADM_ID > 50]
    frequent_shielding_diagnoses_df.reset_index(inplace=True, drop=False)
    frequent_shielding_diagnoses_df["ICD9"] = frequent_shielding_diagnoses_df["ICD9"].map(insert_period)
    ...
    frequent_shielding_diagnoses_df[["ICD9"]].to_csv("SHIELDING_CODES.csv", index=False, header=False)
